...
export const LIMITS_TOKEN_PATTERN =
  /^(rounded(?:-(?:tl|tr|br|bl|t|r|b|l))?(?:-.+)?|w-.+|h-.+|min-w-.+|max-w-.+|min-h-.+|max-h-.+|size-.+)$/
export const FLEX_TOKEN_PATTERN =
  /^(flex|inline-flex|grid|inline-grid|flex-(?:row|row-reverse|col|col-reverse|wrap|wrap-reverse|nowrap|1|auto|initial|none|\[.+\])|basis-.+|grid-cols-.+|grid-rows-.+|col-.+|row-.+|auto-cols-.+|auto-rows-.+|items-.+|justify-.+|content-.+|self-.+|place-(?:items|content|self)-.+|gap(?:-[xy])?-.+|grow(?:-.+)?|shrink(?:-.+)?|order-.+)$/
...
export function sizeModeLabel(mode, axis, isFlexMainAxis = false) {
  if (mode === 'Fill') {
    if (isFlexMainAxis) return 'Fill (flex-1)'
    return axis === 'w' ? 'Fill (w-full)' : 'Fill (h-full)'
  }
...
export function guessDirection(tokens) {
  ...
  if (tokens.includes('flex-row-reverse')) return 'row-reverse'
  if (tokens.includes('flex-col-reverse')) return 'col-reverse'
...
export function getWrapToken(tokens) {
  return findToken(tokens, /^flex-(nowrap|wrap|wrap-reverse)$/)
}

export function guessWrap(tokens) {
  return getWrapToken(tokens) || 'flex-nowrap'
}

export function getGapValue(token, axis = '') {
  const prefix = axis ? `gap-${axis}` : 'gap'
  if (!token.startsWith(`${prefix}-`)) return ''
  return unwrapArbitraryValue(token.slice(prefix.length + 1))
}

export function buildGapToken(value, axis = '') {
  const trimmedValue = value.trim()
  if (!trimmedValue) return ''
  if (/^gap(?:-[xy])?-.+/.test(trimmedValue)) return trimmedValue

  const normalizedValue = normalizePaddingValue(trimmedValue)
  return normalizedValue ? `gap${axis ? `-${axis}` : ''}-${normalizedValue}` : ''
}

function isReverseDirection(direction) {
  return direction === 'row-reverse' || direction === 'col-reverse'
}

function isColumnDirection(direction) {
  return direction === 'col' || direction === 'col-reverse'
}

function flipAlignmentValue(value) {
  if (value === 'start') return 'end'
  if (value === 'end') return 'start'
  return value
}

export function visualToAlignmentTokens({ horizontal, vertical, direction = 'row', wrap = 'flex-nowrap' }) {
  const isColumn = isColumnDirection(direction)
  const mainValue = isColumn ? vertical : horizontal
  const crossValue = isColumn ? horizontal : vertical
  const normalizedMainValue = isReverseDirection(direction) ? flipAlignmentValue(mainValue) : mainValue
  const normalizedCrossValue = wrap === 'flex-wrap-reverse' ? flipAlignmentValue(crossValue) : crossValue

  return {
    justify: normalizedMainValue,
    items: normalizedCrossValue,
    content: normalizedCrossValue,
  }
}

function alignmentTokensToVisual({ justify, items, direction = 'row', wrap = 'flex-nowrap' }) {
  const isColumn = isColumnDirection(direction)
  const visualMainValue = isReverseDirection(direction) ? flipAlignmentValue(justify) : justify
  const visualCrossValue = wrap === 'flex-wrap-reverse' ? flipAlignmentValue(items) : items

  return {
    horizontal: isColumn ? visualCrossValue : visualMainValue,
    vertical: isColumn ? visualMainValue : visualCrossValue,
  }
}

export function getAlignmentState(tokens, direction = 'row', wrap = 'flex-nowrap') {
  const justifyValue = getTokenValue(
    findToken(tokens, /^justify-(start|center|end|between|around|evenly|baseline|normal|stretch)$/),
    'justify'
  )
  const itemsValue = getTokenValue(findToken(tokens, /^items-(start|center|end)$/), 'items')

  const visualAlignment = alignmentTokensToVisual({
  ...
  const horizontal =
    visualAlignment.horizontal === 'start' ||
    visualAlignment.horizontal === 'center' ||
    visualAlignment.horizontal === 'end'
      ? visualAlignment.horizontal
      : ''
  const vertical =
    visualAlignment.vertical === 'start' ||
    visualAlignment.vertical === 'center' ||
    visualAlignment.vertical === 'end'
      ? visualAlignment.vertical
      : ''

  const matrixValue =
    horizontal && vertical
      ? `${vertical === 'start' ? 'top' : vertical === 'center' ? 'middle' : 'bottom'}-${
          horizontal === 'start' ? 'left' : horizontal === 'center' ? 'middle' : 'right'
        }`
      : null
...
export function translateChildTokensOnFlexOff(tokens, isColumnDirection) {
  const fillFallback = isColumnDirection ? 'h-full' : 'w-full'
  return tokens.flatMap((token) => {
    if (token === 'flex-1') return [fillFallback]
    if (/^flex-(none|auto|initial)$/.test(token)) return []
    return [token]
  })
}

export function translateChildTokensOnFlexOn(tokens, isColumnDirection) {
  const fillSource = isColumnDirection ? 'h-full' : 'w-full'
  const fixedMainAxisPattern = isColumnDirection
    ? /^h-(?!(full|fit)$).+$/
    : /^w-(?!(full|fit)$).+$/
  const mapped = tokens.map((token) => (token === fillSource ? 'flex-1' : token))
  const hasFlexToken = mapped.some((t) => /^flex-(1|none|auto|initial)$/.test(t))
  const hasFixedMainAxis = mapped.some((t) => fixedMainAxisPattern.test(t))
  return !hasFlexToken && hasFixedMainAxis ? [...mapped, 'flex-none'] : mapped
}

export function translateChildTokensOnSwitchToGrid(tokens) {
  return tokens.filter((token) => !/^flex-(1|none|auto|initial)$/.test(token))
}
